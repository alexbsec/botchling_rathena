...
bool pc_authok(map_session_data *sd, uint32 login_id2, time_t expiration_time, int32 group_id, struct mmo_charstatus *st, bool changing_mapservers)
{
	...
	botchling_emit_login(sd->status.account_id, sd->status.char_id, ip, map_name);
...
bool pc_takeitem(map_session_data *sd,flooritem_data *fitem)
{
	...
	auto map_name = mapindex_id2name(sd->mapindex);
	botchling_emit_take_item(sd->status.account_id, sd->status.char_id, fitem->item.nameid, fitem->item.amount, map_name);
	return true;
}
...
enum e_setpos pc_setpos(map_session_data* sd, uint16 mapindex, int32 x, int32 y, clr_type clrtype)
{
	...
	auto to_map_name = mapindex_id2name(mapindex);
	...
	if( sd->state.changemap ) { // Misc map-changing settings
		...
		auto curr_map_name = mapindex_id2name(sd->mapindex);
		botchling_emit_map_chg(sd->status.account_id, sd->status.char_id, x, y, curr_map_name, to_map_name);
	}

	if (!sd->state.changemap) {
		botchling_emit_tp(sd->status.account_id, sd->status.char_id, x, y, to_map_name);
	}
